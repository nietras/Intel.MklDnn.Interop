...
{
    public class CblasMethodWrapper
    {
        readonly CblasTypeWrapper m_wrapper;
        StringBuilder m_sb = new StringBuilder();

        public CblasMethodWrapper()
            : this(new CblasTypeWrapper())
        { }

        public CblasMethodWrapper(CblasTypeWrapper wrapper)
        {
        ...
        public string WrapMethod(Method method)
        {
            ...
            var implReturn = method.ReturnType == "void" ? string.Empty : "return ";

            var mgdReturnType = m_wrapper.WrapType(method.ReturnType).Trim();
            var mgdParameters = method.Parameters.Select(p => (p.IsStaticArray ? p.Type.Replace("const", string.Empty).Trim() + "*" : m_wrapper.WrapType(p.Type, method.Name)) + " " + p.DecoratedName);
            m_sb.AppendFormat("{0} {1}({2})", mgdReturnType, CblasHelper.WrapMethodName(method.Name),
            ...
            if (method.Parameters.Any(p => p.IsStaticArray))
            {
                ...
                var castParametersToNative = method.Parameters.Select(p => p.IsStaticArray ? p.Name + "_" : m_wrapper.CastTypeToNtv(m_wrapper.WrapType(p.Type, method.Name), p.Name));
                var methodCall = string.Format("{0}({1})", method.Name, string.Join(", ", castParametersToNative));
                m_sb.AppendFormat("    {0}{1};", implReturn, m_wrapper.CastTypeToMgd(method.ReturnType, methodCall));
                m_sb.AppendLine();
                m_sb.Append("}");
            }
            else
